class DirectinvitesController < ApplicationController
...
#  end
  ...
  def new
    @directinvite = Directinvite.new
    @directinvite.room_id = params[:format] #:format instead of :room_id for some reason
    @directinvite.sender_id = current_user.id
    @directinvite.save

    @possible_invitees = Array.new
    @rooms = current_user.including_rooms
    #@rooms.each {|r| @possible_invitees += r.included_users}
